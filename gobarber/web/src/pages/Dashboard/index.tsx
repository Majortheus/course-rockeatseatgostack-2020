...
import { isToday, isSameDay, isWeekend, nextMonday, format, parseISO, isAfter } from 'date-fns';
...
const Dashboard: React.FC = () => {
  const nextBusinessDay = useMemo(() => {
    return isWeekend(new Date()) ? nextMonday(new Date()) : new Date();
  }, []);

  const [selectedDate, setSelectedDate] = useState(nextBusinessDay);

  const isItToday = useMemo(() => {
    return isToday(selectedDate);
  }, [selectedDate]);

  const isItNextBusinessDay = useMemo(() => {
    return isSameDay(nextBusinessDay, selectedDate);
  }, [nextBusinessDay, selectedDate]);
    ...
    <Container>
      ...
      <Content>
        <Schedule>
          ...
          <p>
            {isItToday && <span>Hoje</span>}
            {isItNextBusinessDay && <span>Próxima dia útil</span>}
            <span>{selectedDateAsText}</span>
            <span>{selectedWeekDay}</span>
          </p>

          {(isItToday || isItNextBusinessDay) && nextAppointment && (
            <NextAppointment>
